...
absviclgadata <- absviclgadata %>% mutate(Region = sub("(.*)\\(.*","\\1",Region)) %>% mutate(Region = sub("(.*)\\s.*","\\1",Region))
...
#this shows that there are a total of 290 different data items! majority of this data is collected annually, however some of these data items have missing data items
...
#this has left us with only 37 actual different data points, when we started with 290!! 
absviclgadata_unique_g620 <- absviclgadata_unique_g6 %>% filter(lga_count > 20)
#of these 37, it must be noted that only 33 of these data items have over 20 points of data. Of these, majority of them are actually vehicle statistics. maybe this analysis should be on whether the number of vehicles in the area have a significant effect on rental price
```

```{r}
#lets do that! it must be noted that although 6 points of data for each LGA does not provide very reliable information, however expanding this to over 70 different LGAs can provide a reasonable conclusion if there is a significant pattern between all of them.
mvcdata <- absviclgadata %>% dplyr::filter(grepl("MVC",MEASURE)) %>% select(-MEASURE)
#lets look at a sample of the data for one LGA, and plot it against its median rental price. the motor vehicle census data is taken at the start of each month.
#in order to do this, we must first match the data from the rental data to the mvc data. the rental data is taken quarterly, starting from march. we assume that the information from the mvc taken at january doesn't change significantly by march. therefore we match the march rental data to the january mvc data.
mvcdata <- mvcdata %>% mutate(Time=dmy(paste("1-3-",Time)))
mvcdata <- filter(mvcdata,`Data item` %in% absviclgadata_unique_g620$`Data item`) #population data
medianrentps <- medianrentps %>% dplyr::rename(medianrent=values)
mvc_rent <- mvcdata %>% left_join(medianrentps,by=c("Region"="Suburb","Time"="date"))
#we want time to start at 0 for regression purposes
mvc_rent <- mvc_rent %>% mutate(Time=(year(Time)-2011))

```

```{r}
#we plot all our explanatory data to see what we have
#l <- ggplot(mvcdata,aes(x=Time,y=Value,colour=Region))+geom_line()+facet_wrap(~`Data item`)
#ggplotly(l)
#we then begin plotting linear models to each of the categories to see which provides the best fit
library(purrr)

mvc_rent_bylga <- mvc_rent %>% group_by(Region,`Data item`) %>% nest()
mvc_rent_bylga <- mvc_rent_bylga %>% dplyr::mutate(model=purrr::map(data, ~possibly(lm,NULL)(medianrent ~ Value, data=.)))
mvc_coeffs <- mvc_rent_bylga %>% unnest(model %>% purrr::map(broom::tidy))
mvc_coeffs <- mvc_coeffs %>% select(Region,`Data item`,term,estimate) %>% spread(term, estimate) #%>% rename(intercept=`(Intercept)`)
mvc_rsquared <- mvc_rent_bylga %>% unnest(model %>% purrr::map(broom::glance)) %>% select(Region,`Data item`,r.squared)
mvc_coeffs <- mvc_coeffs %>% left_join(mvc_rsquared,by=c("Region","Data item")) %>% dplyr::rename(intercept=`(Intercept)`)
mvc_coeffs_test <- mvc_coeffs %>% arrange(-r.squared)
mvc_coeffs_test
```

```{r}
#now we plot the intercept and value to see if there is any grouping between LGAs
mvc_coefplot <- ggplot(mvc_coeffs,aes(x=intercept,y=Value,colour=`Data item`))+geom_point()
ggplotly(mvc_coefplot)
```

```{r}
#Interesting! the values seem to mainly be centered around (300,0.0015), and they all the points seem to be linearly distributed amongst an arbitrary axis depending on the type of vehicle. how about we take the highest average r.squared for each data item
mvc_bestavgr <- mvc_coeffs %>% group_by(`Data item`) %>% dplyr::summarise(avgr.squared = mean(r.squared,na.rm=TRUE),var =var(r.squared,na.rm=TRUE)) %>% arrange(-var)
mvc_bestavgr
```

```{r}
#interesting, the highest average r.squared is for diesel vehicles and also has the smallest variance amongst the top 10 highest average r.squared.
#now lets look at the grouping of the linear models
mvcdiesel <- mvc_coeffs %>% filter(`Data item`=="Diesel (no.)")
mvcdiesel_rsqrmed <- median(mvcdiesel$r.squared)
mvcdiesel_avg <- mvcdiesel %>% group_by(`Data item`) %>% dplyr::summarise(avgval = mean(Value,na.rm=TRUE),avgint = mean(intercept,na.rm=TRUE))
mvcdiesel_med <- mvcdiesel %>% filter(r.squared == mvcdiesel_rsqrmed)
mvc_dieselplot <- ggplot(mvcdiesel,aes(x=intercept,y=Value))+geom_point()+geom_point(data=mvcdiesel_med,aes(x=intercept,y=Value),colour="Red")+geom_point(data=mvcdiesel_avg,aes(x=avgint,y=avgval),colour="Green")
mvc_dieselplot
#the median model is highlighted in red, and the average model is highlighted in green
#consider time in months? mismatch in times.
```
```{r}
#lets take the median model and see how well it can explain every single lga, and compare it against the average model.
mvc_rentpred <- mvc_rent %>% filter(`Data item`=="Diesel (no.)") %>% mutate(pred=mvcdiesel_avg$avgint+Value*mvcdiesel_avg$avgval)
glimpse(mvc_rentpred)
mvc_dieselpred <- ggplot(mvc_rentpred,aes(x=Time,y=medianrent,colour=Region))+geom_point()+geom_line(aes(x=Time,y=pred))
ggplotly(mvc_dieselpred)
```
```{r}
#we can see from the plot that the estimated model, taken from the average of the diesel models is conservative, and estimates reasonably well for a large majority of the LGAs, but not all of them.
```
...
gdp <- gdp %>% filter(MEASURE == "USD_CAP")
gdp <- gdp[,c(1,6,7)]
#historical gdp plot
...
vic <- get_map(location = c(lon=144.9631, lat=-37.8136), zoom=7)
